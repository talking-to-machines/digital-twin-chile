"""
Arm A — Corrected Baseline Prompt Template
AIPOP Chile — Digital Interview Pipeline

Architecture: single-stage, monolithic (same as original prompt_template.py).

Changes from original prompt_template.py:
- REGLA DE FORMATO CRÍTICA added to both user prompts (prevents symbol/category mixing).
- REGLA DE INFERENCIA CONSERVADORA added to voting preference prompt (prevents modal defaults).
...
Pipeline calls (same as original):
  Call 1: system=x_digital_twin_system_prompt,
           user=x_digital_twin_entity_geographic_user_prompt
  Call 2: system=x_digital_twin_system_prompt,
           user=x_digital_twin_voting_preference_wo_voting_results_user_prompt

Output parsing: regex on **field: value** lines (same as digital_twin_config.py patterns).
"""
...
# ─── Entity and Geographic Interview ─────────────────────────────────────────
...
Correcto:
**symbol: PP12**
**category: Evolución Política (EVOPOLI)**
...
REG3) DE ATACAMA
REG4) DE AYSEN DEL GENERAL CARLOS IBÁÑEZ DEL CAMPO
REG5) DE COQUIMBO
...
x_digital_twin_voting_preference_wo_voting_results_user_prompt = """REGLA DE INFERENCIA CONSERVADORA:
Solo seleccione una categoría para edad, educación, ocupación, estado civil, ingresos u orientación política si el perfil proporciona evidencia clara y directa.
...
OCCUP3) Empleado u obrero del sector público (Gob. Central o Municipal, excluye Fuerzas Armadas)
...
Vpaindv6) votó por un candidato del Partido por la Democracia (PPD) en las elecciones legislativas de 2021
Vpaindv7) votó por un candidato de Unión Demócrata Independiente (UDI) en las elecciones legislativas de 2021
Vpaindv8) votó por un candidato de Renovación Nacional (RN) en las elecciones legislativas de 2021
Vpaindv9) votó por un candidato del Partido Republicano (PLR) en las elecciones legislativas de 2021
...
Und1) No hay posibilidad de que esta persona cambie de opinión — probabilidad: 0
Und2) Es muy improbable que esta persona cambie de opinión — probabilidad: 0,15
Und3) Es poco probable que esta persona cambie de opinión — probabilidad: 0,3
Und4) Probabilidad del 50% de que esta persona cambie de opinión — probabilidad: 0,5
